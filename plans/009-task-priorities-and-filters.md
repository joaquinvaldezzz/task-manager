...
> in `plans/README.md`.
>
> **Drift check (run first)**: `git diff --stat 0b9ec9c..HEAD -- app/Models/Task.php resources/js/types/task.ts resources/js/pages/dashboard/tasks-table.tsx`
...
- In `resources/js/types/task.ts:6`:
  ...
  ```
- In `database/migrations/2026_01_19_164509_create_tasks_table.php`:
...
- `resources/js/types/task.ts` (add `priority` to `TaskFormData`)
...
- Commit message: `feat(tasks): support task priority and status filter tabs`
...
Add a priority `<Select>` or radio group offering "Low", "Medium", "High" options with default "medium".
...
2. Add filter tabs (`All`, `Active`, `Completed`) above the table using `Tabs` from `@/components/ui/tabs` or buttons.
3. Filter tasks array:
   ...
   ```
4. Render colored `Badge` for `task.priority` (`high`: destructive/amber, `medium`: secondary/default, `low`: outline/muted).
