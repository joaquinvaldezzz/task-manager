...
const buttonClassNames =
  "relative flex size-(--cell-size) items-center justify-center rounded-lg text-base text-foreground not-in-data-selected:hover:bg-accent disabled:pointer-events-none disabled:opacity-64 sm:text-sm [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='opacity-'])]:opacity-80 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4";
...
function Chevron({ className = "", orientation = "down" }: ChevronProps) {
  if (orientation === "left") {
    return <ChevronLeftIcon className={cn(className, "rtl:rotate-180")} aria-hidden="true" />;
  }

  if (orientation === "right") {
    return <ChevronRightIcon className={cn(className, "rtl:rotate-180")} aria-hidden="true" />;
  }

  return <ChevronsUpDownIcon className={className} aria-hidden="true" />;
}
...
function Calendar({
  ...
  const defaultClassNames = {
    ...
    day_button: cn(
      buttonClassNames,
      "outline-none focus-visible:z-1 focus-visible:ring-[3px] focus-visible:ring-ring/50 in-data-disabled:pointer-events-none in-data-disabled:text-muted-foreground/70 in-data-disabled:line-through in-data-outside:text-muted-foreground/70 in-data-selected:bg-primary in-data-selected:text-primary-foreground in-data-selected:in-data-outside:text-primary-foreground in-[.range-end:not(.range-start)]:rounded-s-none in-[.range-middle]:rounded-none in-[.range-middle]:in-data-selected:bg-accent in-[.range-middle]:in-data-selected:text-foreground in-[.range-start:not(.range-end)]:rounded-e-none in-[[data-selected]:not(.range-middle)]:transition-[color,background-color,border-radius,box-shadow]",
    ),
    ...
    today:
      "*:after:pointer-events-none *:after:absolute *:after:bottom-1 *:after:start-1/2 *:after:z-1 *:after:size-0.75 *:after:-translate-x-1/2 *:after:rounded-full *:after:bg-primary [&[data-selected]:not(.range-middle)>*]:after:bg-background [&[data-disabled]>*]:after:bg-foreground/30 *:after:transition-colors",
